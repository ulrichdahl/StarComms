...
describe('buildReport', () => {
  ...
  it('surfaces sweep counts alongside the verdict', () => {
    const r = buildReport(
      fakeFleet([bot('alfa')]),
      { hailsForceClosed: 2, vesselsPresent: 3, registryEntries: 1 },
      new Date(),
    );
    expect(r.sweep.hailsForceClosed).toBe(2);
    expect(r.sweep.vesselsPresent).toBe(3);
    expect(r.sweep.registryEntries).toBe(1);
  });
});
